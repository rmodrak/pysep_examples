...
..note:
  ...
  This simple installation works for our purposes. For the most detailed and
  up-to-date instructions for installing PySEP see:
  https://pysep.readthedocs.io/en/latest/
...
5. After the waveform downloads finish, SAC files and corresponding 
   MTUQ "weight files" will be present in:
   2006-10-09T013528_NORTH_KOREA/

   These waveforms can then be used with `mtuq_script.py` to reproduce the 
   results in `mtuq_output/`.

   To run the MTUQ script, it is necessary to deactivate the PySEP environment
   and activate a new environment in which MTUQ installed. For instructions on 
   installing MTUQ see:
